...
/**
 * Function that reformats the given date to a more user-friendly one
 *
 * @param  {String}    date                     The tweet date
 * @return {String}                             The formatted date (dd/mm/yyyy)
 * @api private
 */
var _reformatDate = function(date) {
    var d = new Date(date);
    var day = d.getDate();
    var month = d.getMonth()+1;
    var year = d.getFullYear();
    var dayOfTheWeek = d.getUTCDay(d);
    var weekdays = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    return weekdays[dayOfTheWeek] + ', ' +[day, month, year].join('/');
};
...
var _createTweetModel = function(record) {
    ...
    var created_at = _reformatDate(record.created_at);
