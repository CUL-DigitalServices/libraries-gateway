...
var registerRoutes = function(root) {
    ...
    _app.get('/api/search/facets', require('../controllers/api/search').getFacetsFromResults);
    ...
    _app.get('/find-a-resource/facets', resourcesController.getResultFacets);
...
var createServer = module.exports.createServer = function(callback, attempts) {
    if (!callback) {
        log().info('Trying to spin up ' + config.app.title + ' at ' + config.server.protocol + '://' + config.server.host + ':' + config.server.port);
    }

    // Keep track of the attempts of setting up a new webserver
    attempts = attempts || 0;
    if (!callback && attempts === 10) {
        return log().warn('Could not spin up ' + config.app.title + ' at ' + config.server.protocol + '://' + config.server.host + ':' + String(config.server.port) + ' in 10 attempts');
    }
    ...
    server.once('listening', function() {
        ...
        var basePath = util.format(config.server.protocol + '://%s/api', config.server.host);
        ...
        if (!callback) {
            ...
            log().info(config.app.title + ' started at ' + config.server.protocol + '://' + config.server.host + ':' + String(config.server.port) + ' with ' + attempts + ' ' + attemptsString);
        }
    ...
    server.once('error', function(err) {
        ...
        if (callback) {
            return callback('Could not spin up ' + config.app.title + ' at ' + config.server.protocol + '://' + config.server.host + ':' + String(config.server.port) + ' in 10 attempts');
        }
