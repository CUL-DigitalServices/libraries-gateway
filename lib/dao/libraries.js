...
var getLibraries = module.exports.getLibraries = function(callback) {
    ...
    // Build up the query
    var query = 'SELECT ';
    query += 'L.library_id, L.library_code, L.library_name, L.admits, L.stock, L.special_collections, L.services, ';
    query += 'LA.address, AT.address_type_name ';
    query += 'FROM libraries L ';
    query += 'INNER JOIN library_addresses LA ON (L.library_id = LA.library_id) ';
    query += 'LEFT JOIN address_types AT ON (AT.address_type_code = LA.address_type_code) ';
    query += 'ORDER BY L.library_id ASC';
