...
var MySQL = require('lg-util/lib/mysql');

var Coords = require('../models/libraries/coords').Coords;
var Library = require('../models/libraries/library').Library;
var Parent = require('../models/libraries/library').Parent;
var Staff = require('../models/libraries/library').Staff;
var Type = require('../models/libraries/library').Type;
...
var getLibraries = module.exports.getLibraries = function(callback) {
    ...
    MySQL.connectWithDB(connectionParams)

        // Request the libraries from the database
        .then(function(connection) {
            ...
            query += 'L.new_opening, L.admits, L.stock, L.special_collections, L.services, L.see, L.see_also, DBS.dbase_name, ';
            ...
            connection.query(query, function(err, resultset, fields) {
                ...
                for (var i=0; i<rows.length; i++) {
                    ...
                    var type = new Type(row.type_code, row.type_name);

                    var opening_hours = row.new_opening;
                    ...
                    // Create a model for the parent library
                    var see = null;
                    if (row.see) {
                        see = new Parent(row.see, slugs(row.see).toLowerCase());
                    }

                    // Create a model for an affiliated library
                    var see_also = null;
                    if (row.see_also) {
                        see_also = new Parent(row.see_also, slugs(row.see_also).toLowerCase());
                    }
                    ...
                    try {
                        ...
                        if (getAddressValuesByType(addresses, 'latlng')) {
                            ...
                            coords = new Coords(coords[0], coords[1]);
                        }
                    ...
                    try {
                        if (row.staff !== null) {
                            ...
                            _.each(staffObject, function(member) {
                                ...
                                var staffModel = new Staff(title, name, email, position, display_email, display_title);
                    ...
                    var library = new Library(id, code, name, url, type, coords, email, address, telephone, fax, web, map,
                        facebook, blog, twitter, opening_hours, admits, stock, special_collections, services, see, see_also, staff, dbase);
        ...
        // Return an error if setting up a connection failed
        .catch(function(err) {
            return callback(err);
        });
};
...
var getLibraryBySlug = module.exports.getLibraryBySlug = function(slug, callback) {
     ...
     */
    getLibraries(function(error, libraries) {
        if (error) {
