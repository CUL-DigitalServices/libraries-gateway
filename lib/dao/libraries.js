    ...
    util.connectWithDB(connectionParams, function(err, connection) {
        ...
        connection.query(query, function(err, resultset, fields) {
            ...
            for (var i=0; i<rows.length; i++) {
                ...
                var id = rows[i].library_id;
                var code = rows[i].library_code;
                var name = rows[i].library_name;
                var url = slugs(rows[i].library_name).toLowerCase();

                var type = new libraryModel.Type(rows[i].type_code, rows[i].type_name);

                var opening_hours = rows[i].opening_hours;
                var admits = rows[i].admits;
                var stock = rows[i].stock;
                var special_collections = rows[i].special_collections;
                var services = rows[i].services;
                var dbase = rows[i].dbase_name;

                // Try to parse the addresses
                try {
                    var addresses = JSON.parse(rows[i].addresses);
                ...
                // Execute callback when parsing fails
                    ...
                    return callback("Parsing library data failed (1)");
                }

                // Try to parse the staff
                try {
                    if (rows[i]) {
                        if (rows[i].staff && rows[i].staff != null) {
                            var staff = JSON.parse(rows[i].staff);
                        }
                    }
                } catch (e) {
                    return callback("Parsing library data failed (2)");
                }
                ...
                var library = new libraryModel.Library(id, code, name, url, type, coords, email, address, telephone, fax, web, map, 
                    opening_hours, admits, stock, special_collections, services, staff, dbase);
