...
var getResourceDetail = exports.getResourceDetail = function(req, res) {
    ...
    navigationController.getContent(req, res, function(err, navigation) {
        ...
        _getResourceResults(url, function(error, response) {
            ...
            var title = params.data.title.join(', ');
            var authors = params.data.author.join(', ');
            var shareUrl = params.shareUrl = req.header('host') + '/find-a-resource/' + api + '/' + params.data.id;
            var words = title.split(' ');
            if (words.length >= 10) {
                // Cut the title off after the tenth word
                title = words.slice(0, 10).join(' ') + '...';
            }
            params.shareText = 'I\'ve just found "'+ title +'" by '+ authors +' @libatcam. ' + shareUrl;
