...
var BlogsController = module.exports.BlogsController = function() {
    ...
    that.getContent = function(req, res) {
        ...
        // If the entries are not expired
            ...
            _createPagination(res, page, itemsPerPage, _entries.length, function(error, tplPagination) {
                if (error) {
                    log().error(error);
                    return that.renderTemplate();
                }

                // Add the pagination to the template data
                data.partials.tplPagination = tplPagination;
    ...
    /**
     * Function that generates the pagination
     *
     * @param  {Object}    res                  The response object
     * @param  {Number}    currentPage          The current page
     * @param  {Number}    itemsPerPage         The number of items per page
     * @param  {Number     totalItems           The number of results
     * @param  {Function}  callback             Standard callback function
     * @param  {Error}     callback.error       The thrown error
     * @param  {String}    callback.template    The generated pagination template
     * @api private
     */
    var _createPagination = function(res, currentPage, itemsPerPage, totalItems, callback) {
        try {
            ...
            res.render('partials/pagination', paginationOptions, function(error, template) {
                if (error) {
