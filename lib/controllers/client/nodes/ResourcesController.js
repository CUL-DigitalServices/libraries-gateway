...
var ResourcesController = module.exports.ResourcesController = function() {
    ...
    that.getContent = function(req, res) {
        ...
        // Show the empty search view if no query has been specified
        if (_.isEmpty(req.query) || !req.query.q) {

            // Render the body for the resources
            return that.renderTemplate(req, res, parameters, 'nodes/find-a-resource', 'find-a-resource');

        // Render the results after a query has been executed
        } else {
            ...
            SearchAPI.getResults(req.query, function(err, results) {
                if (err) {
                ...
                try {
                    ...
                    var _renderItemsTemplate = function() {
                        ...
                        var templateParameters = _.clone(parameters);
                        ...
                        res.render('partials/find-a-resource/api-results', templateParameters, function(err, tplResults) {
                            ...
                            parameters.partials.tplResults = tplResults;

                            // Render the body for the resources
                            return that.renderTemplate(req, res, parameters, 'nodes/find-a-resource-results', 'find-a-resource-results');
                        });
                    };
