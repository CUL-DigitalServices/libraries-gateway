    ...
    that.renderTemplate = function(req, res, data, template, templateID) {
        ...
        res.render(template, parameters, function(err, tplBody) {
            if (err) {
                ...
                return res.status(500).send('Error while rendering the node template');
            }
            ...
            return res.render('index', parameters, function(err, tplIndex) {
                if (err) {
                    ...
                    return res.status(500).send('Error while rendering the index template');
                }
                return res.status(200).send(tplIndex);
            });
        });
    };
};
