...
/**
 * Function that constructs the request query string
 *
 * @param  {Object}     parameters       The query parameters (e.g. 'id', 'branch'...)
 * @param  {String[]}   extraParams      Extra API bound request parameters (e.g. 'output=xml')
 */
var constructRequestQueryString = module.exports.constructRequestQueryString = function(parameters, extraParams) {
    ...
    if (parameters['id']) {
        queryString.push(util.format('id:%s', parameters['id']));
        ...
        if (parameters['branch']) {
            extraParams.push(util.format('branch=%s', parameters['branch']));
        }

        // Check if the current page is set (e.g. 2)
        if (parameters['page']) {
            extraParams.push(util.format('curpage=%s', parameters['page']))
        }

        // Check if facets have been set
        if (parameters['ref']) {
            extraParams.push(util.format('ref=%s', _sanitizeFacets(parameters['ref'])));
        }
    }

    // Construct the url for the request
    extraParams.push('q=' + encodeURIComponent(queryString.join(' ')));
    var queryString = extraParams.sort().join('&');
 ...
 *
 * @param  {Object}     feedbacks       The Aquabrowser request feedbacks
 * @param  {Object}     parameters      The query parameters
 * @return {Array}                      The returned facetsOverview collection
 */
var generateAppliedFilterCollection = module.exports.generateAppliedFilterCollection = function(feedbacks, parameters) {

    var overview = [];
    var toIgnore = ['api', 'facet', 'id', 'page', 'q'];

    if (feedbacks.RefineCrumbtrail) {

        var usedFacets = feedbacks.RefineCrumbtrail.reflbl;
        if (!_.isArray(usedFacets)) {
            usedFacets = [ usedFacets ];
        }

        if (parameters.ref) {
            var refs = _sanitizeFacets(parameters['ref']).split(';');
            _.each(refs, function(ref) {
                var key = _.find(usedFacets, function(facet) { return facet.id === ref; });
                var value = _.difference(refs, ref).join(';');

                var stripped = _.clone(parameters);
                delete stripped['ref'];
                ...
                var url = querystring.stringify(stripped).replace(/\%3B/g, ';');
                overview.push({'label': key.kw, 'url': url});
            });
        }
    }
 ...
 *
 * @param  {Object}     record      Object containing record data
 * @return {String[]}               Collection of content types
 */
...
/**
 * Returns the location of the eResource
 *
 * @param  {Object}     record      Object containing record data
 * @return {String}                 The location of the eResouce (e.g. http://ezproxy.lib.cam.ac.uk:2048/login?url=http://online.sagepub.com/0963-6625)
 */
var getEResource = module.exports.getEResource = function(record) {
    try {
        var location = null;
        var marc = 'df856';
        if (record.d && record.d[0]) {
            var data = record.d[0];
            if (data[marc] && data[marc][marc]) {
                location = _.filter(data[marc][marc], function(node) { return node.key === 'u' });
                if (location.length) {
                    location = location[0]['_'];
                }
            }
        }
        return location;
...
/**
 * Function that sanitizes the facets from the queystring
 *
 * @param  {String}     keys    String containing the selected facets (e.g. 20;24;3888;invalid_ref;64£$%)
 * @return {String}             Sanitized string containing the facet references (e.g. 20;24;3888)
 * @api private
 */
var _sanitizeFacets = function(keys) {
    var refs = keys.split(';');
    _.each(refs, function(ref, index) {
        if (ref.search(/[^0-9]/i) !== -1) {
            delete refs[index];
        }
    });
    return _.compact(refs).join(';');
}
