    ...
    try {
        ...
        if (_.isArray(items[marc])) {
            _.each(items[marc], function(item) {
                var branch = ResourceModelFactory.createBranchModel({
                    'name': _.where(item[marc], {'key': '9'})[0]['_'],
                    'classmark': _.where(item[marc], {'key': 'h'})[0]['_'],
                    'status': null,
                    'numberOfItems': null
                });
            ...
            var branch = ResourceModelFactory.createBranchModel({
                'name': _.where(items[marc][marc], {'key': '9'})[0]['_'],
                'classmark': _.where(items[marc][marc], {'key': 'h'})[0]['_'],
                'status': null,
                'numberOfItems': null
            });
            branches.push(branch);
        ...
        if (branches && !branches.length) {
            ...
            branches = branches.slice(0, config.nodes['find-a-resource'].settings.numberOfHoldingsShown);
        }
        return branches;
    } catch(error) {
        log().error(error);
