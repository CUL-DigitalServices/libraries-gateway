...
var getFacetsFromResults = module.exports.getFacetsFromResults = function(parameters, callback) {
    ...
    var options = AquabrowserUtil.constructRequestOptions(uri, parameters, extraParams);
                            ...
                            try {
                                if (recordsCreated === recordsToCreate) {
                                    ...
                                    _getFacets(results.root.refine.d, parameters, function(error, facetsCollection) {
                                        ...
                                        filters = AquabrowserUtil.generateAppliedFilterCollection(parameters);
            ...
            if (_.isObject(facetType)) {
                ...
                if (facetType.kw) {
                    ...
                    _.each(facetType.kw, function(facet) {
                        ...
                        var facetRef = facet.i;
                        var facetUrl = searchUtil.createFacetUrl(parameters, util.format('ref:%s', facet.i), facetLabel);

                        // Create a new facet model
                        var facetModel = new FacetModel.Facet(facetLabel, facetAmount, facetUrl, facetRef);
