...
var getResults = module.exports.getResults = function(parameters, callback) {
    ...
    request(options, function(err, res, body) {
        if (err) {
            return callback('An error occurred while fetching Summon data (0)');
            ...
            try {
                ...
                if (body.errors) {
                    ...
                    return callback('An error occurred while fetching Summon data (1)');
                }
                ...
                for (var i=0; i<body.documents.length; i++) {
                    ...
                    // ID
                    if (!item['ID']) {
                        return callback('An error occurred while parsing Summon data (2)');
                    }
                ...
                try {
                    ...
                    if (body.recordCount) {
                        ...
                        var pageNumber = Number(body.query.pageNumber);
                        var pageCount = Number(body.pageCount);

                        // Since Summon only supports 50 pages of results, we need trash all the pages that come after page 50
                        // (will return 'page.number.too.large' error otherwise)
                        if (pageCount > 50) pageCount = 50;

                        var firstPage = Number(1);
                        var lastPage = Number(pageCount);

                        pagination = search.createPaginationModel(parameters, pageNumber, pageCount, firstPage, lastPage);
                    }

                } catch(e) {
                    return callback('An error occurred while fetching Summon data (3)');
                }
            ...
            // When the parsing of the Summon result failed
                ...
                return callback('An error occurred while fetching Summon data (4)');
            }
        }
    });
};
