    ...
    if (body) {
        try {
            for (var i=0; i<body.documents.length; i++) {
                ...
                var author = _getResourceAuthors(item);
                var published = _getResourcePublicationData(item);
                var subject = null;
                var physicalDescription = null;
                var series = null;
                var note = null;
                var contentType = _getPropertyData(item, 'ContentType');
                var links = _getPropertyData(item, 'link');
                ...
                var result = new ResultModel.Result(id, src, extId, title, isbn, eisbn, issn, ssid, author, published, subject, physicalDescription, series, note, contentType, thumbnail, links);
                results.push(result);
            }
        } catch(error) {
            log().error(error);
...
var _getResourceAuthors = function(item) {
    try {
        ...
        if (item['Author_xml'] && item['Author_xml'].length) {
            authors = [];
            _.each(item['Author_xml'], function(author) {
                if (author.fullname) {
...
var _getResourcePublicationData = function(item) {
    try {
        ...
        var publicationTitle = libutil.consolidateValue(item['PublicationTitle']);
        var publicationVolume = libutil.consolidateValue(item['Volume']);
        var publicationIssue = libutil.consolidateValue(item['Issue']);
