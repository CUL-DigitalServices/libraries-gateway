    ...
    if (body) {
        try {
            for (var i=0; i<body.documents.length; i++) {
                ...
                var modelData = {
                    'id': _getPropertyData(item, 'ID'),
                    'src': null,
                    'extId': null,
                    'titles': _getPropertyData(item, 'Title'),
                    'isbn': _getPropertyData(item, 'ISBN'),
                    'eisbn':  _getPropertyData(item, 'EISBN'),
                    'issn': _getPropertyData(item, 'ISSN'),
                    'ssid': _getPropertyData(item, 'SSID'),
                    'authors': _getResourceAuthors(item),
                    'published': _getResourcePublicationData(item),
                    'subjects': null,
                    'series': null,
                    'notes': null,
                    'contentType': _getPropertyData(item, 'ContentType'),
                    'thumbnails': null,
                    'links': _getPropertyData(item, 'link'),
                    'branches': null
                };

                // Thumbnail
                if (item['thumbnail_s']) {
                    modelData.thumbnails = _getPropertyData(item, 'thumbnail_s');
                } else if (item['thumbnail_m']) {
                    modelData.thumbnails = _getPropertyData(item, 'thumbnail_m');
                } else if (item['thumbnail_l']) {
                    modelData.thumbnails = _getPropertyData(item, 'thumbnail_l');
                }
 ...
 * @api private
 */
var _getResourceAuthors = function(item) {
    try {
        var authors = null;
 ...
 * @api private
 */
var _getResourcePublicationData = function(item) {
    try {
