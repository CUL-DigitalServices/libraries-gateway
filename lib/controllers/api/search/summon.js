...
var getResults = module.exports.getResults = function(isSummon, parameters, callback) {
    ...
    var facetsCollection = ['s.ff=ContentType,and,1,5','s.ff=SubjectTerms,and,1,5','s.ff=Language,and,1,5'];
    var queryString = ['s.ps=25', 's.dym=true', 's.debug=true'];
        ...
        if (body.facetFields) {
            ...
            _.each(body.facetFields, function(facetType) {
                ...
                _.each(facetType.counts, function(facet) {
                    ...
                    var property = facet.value;
                    var numItems = facet.count;
                    var url = null;

                    var facetModel = new FacetModel.Facet(property, numItems, url);
