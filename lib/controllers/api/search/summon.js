...
var getResults = module.exports.getResults = function(_queryString, callback) {
    ...
    if (_queryString) {
        ...
        if (_queryString['id']) {
            ...
            console.log('summon: id is set: ' + _queryString['id']);

            // TODO: FIND ID PARAMETERS
            // queryString.push('s.q=' + _queryString['id']);
            ...
            if (_queryString['format'] && _queryString['format'] !== 'all') {
            ...
            if (_queryString['records'] > 0) {
                queryString.push('s.ps=' + _queryString['records']);
            }

            // Check if the current page is set
            if (_queryString['page']) {
                queryString.push('s.pn=' + _queryString['page']);
            }
        }
    }
        ...
        if (err) {
            ...
            try {
                ...
                for (var i=0; i<response.documents.length; i++) {
                    ...
                    var result = new ResultModel.Result(id, title, author, date, contentType, thumbnail);
