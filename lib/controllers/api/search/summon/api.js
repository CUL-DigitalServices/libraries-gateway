...
var getFacetsFromResults = module.exports.getFacetsFromResults = function(parameters, callback) {
    ...
    var options = SummonUtil.constructRequestOptions(null, parameters, extraParams, queryString);
    ...
    if (body) {
        try {
            for (var i=0; i<body.documents.length; i++) {
                ...
                var modelData = {
                    'id': SummonUtil.getPropertyData(item, 'ID'),
                    'src': null,
                    'extId': null,
                    'titles': SummonUtil.getPropertyData(item, 'Title'),
                    'description': null,
                    'isbn': SummonUtil.getPropertyData(item, 'ISBN'),
                    'eisbn':  SummonUtil.getPropertyData(item, 'EISBN'),
                    'issn': SummonUtil.getPropertyData(item, 'ISSN'),
                    'ssid': SummonUtil.getPropertyData(item, 'SSID'),
                    'authors': SummonUtil.getResourceAuthors(item),
                    'published': SummonUtil.getResourcePublicationData(item),
                    'subjects': SummonUtil.getPropertyData(item, 'Discipline'),
                    'series': null,
                    'tags': null,
                    'notes': null,
                    'contentType': SummonUtil.getPropertyData(item, 'ContentType'),
                    'thumbnails': null,
                    'links': SummonUtil.getPropertyData(item, 'link'),
                    'eResource': null,
                    'branches': null
                };
                ...
                // Thumbnail
                if (item['thumbnail_s']) {
                    modelData.thumbnails = SummonUtil.getPropertyData(item, 'thumbnail_s');
                } else if (item['thumbnail_m']) {
                    modelData.thumbnails = SummonUtil.getPropertyData(item, 'thumbnail_m');
                } else if (item['thumbnail_l']) {
                    modelData.thumbnails = SummonUtil.getPropertyData(item, 'thumbnail_l');
                }
