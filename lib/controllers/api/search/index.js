...
var getCombinedResults = exports.getCombinedResults = function(req, res) {
    ...
    var isAquabrowser = (!query.api || query.api === 'aquabrowser');
    var isSummon = (query.api === 'summon');
 ...
 */
var getFacetsFromResults = exports.getFacetsFromResults = function(req, res) {
    ...
    if (!query.api) {
        return res.send(400, {'error': 'Invalid or no api specified'});
    } else if (!query.facet) {
        return res.send(400, {'error': 'Invalid or no facet specified'});
    } else if (!query.q) {
        return res.send(400, {'error': 'Invalid or no query specified'});
    }
    ...
    try {
         ...
         */
        var _parseFacets = function(error, results) {
            if (error) {
                log().error(error);
                return res.send(500, {'error': 'Error while fetching facets'});
            }
        ...
        if (isAquabrowser) {
            AquabrowserAPI.getFacetsFromResults(query, _parseFacets);
        } else if (isSummon) {
            SummonAPI.getFacetsFromResults(query, _parseFacets);
        }
    } catch(error) {
        log().error(error);
        return res.send(500, {'error': 'Error while fetching facets'});
...
var _getAquabrowserResults = function(isAquabrowser, query, callback) {
    AquabrowserAPI.getResults(isAquabrowser, query, function(error, response) {
        if (error) {
...
var _getSummonResults = function(isSummon, query, callback) {
    SummonAPI.getResults(isSummon, query, function(error, response) {
        if (error) {
            log().error(error);
            return callback({'error': error});
        }
