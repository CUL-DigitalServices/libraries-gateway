...
var getResults = exports.getResults = function(opts, callback) {
    ...
    // Default the API to Aquabrowser if no APU has been specified
    if (!opts.api || opts.api !== 'summon') {
        opts = _.extend(opts, {'api': 'aquabrowser'});
    }
    ...
    if (opts.api === 'aquabrowser') {
        AquabrowserAPI.getResults(true, opts, function(err, results) {
            if (err) {
    ...
    // Get the results from Summon
        ...
        SummonAPI.getResults(true, opts, function(err, results) {
            if (err) {
                log().error(err);
                return _resultsCallback(err);
            }
