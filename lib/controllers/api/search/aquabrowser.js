    ...
    try {
        ...
        if (isDetailRequest) {
            _getItemAvailability(extId, function(error, branches) {
                ...
                var result = new ResultModel.Result(id, src, extId, titles, isbn, null, null, null, author, published, subject, series, note, contentType, thumbnail, link, branches);
    ...
    try {
        ...
        if (record.d && record.d[0]){
            ...
            if (data['df100']) {
                ...
                // Loop the authors
                _.each(data['df100'], function(row) {
                    var fullname = null;
    ...
    try {
        ...
        if (record.d && record.d[0]){
            ...
            if (data['df245']) {
                ...
                // Loop the titles
                _.each(data['df245'], function(row) {
                    var title = [];
                    _.each(row, function(item) {
