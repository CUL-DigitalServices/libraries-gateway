                    ...
                    var _doCreateResourceModel = function(recordsToCreate, record) {
                        ...
                        _createResourceModel(isDetailRequest, record, function(err, resource) {
                            ...
                            if (recordsCreated === recordsToCreate) {
                                ...
                                // Get the facets
                                _getFacets(res, function(err, _facets) {
                                    if (err) {
                                        return callback('An error occurred while fetching Aquabrowser data');
                                    }
...
var _createAvailabilityModel = function(availability) {
    ...
    var branch = new ResultsModel.Branch(location, sublocation, status, itemCount, notes);
    return branch;
};
    ...
    try {
        ...
        if (isDetailRequest) {
            ...
            _getItemAvailability(extId, function(err, branches) {
                if (err) {
                    console.log('Error while fetching the availability information');
                }

                // Create a new model for the resource
                var result = new ResultModel.Result(id, extId, title, isbn, null, null, null, author, date, physicalDescription, series, note, contentType, thumbnail, link, branches);
 ...
 */
var _getFacets = function(res, callback) {
