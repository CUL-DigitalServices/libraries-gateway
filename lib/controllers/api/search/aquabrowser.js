            ...
            if (numRecords) {
                ...
                _.each(records, function(record, index) {
                    if (record.fields) {
                        ...
                        var branches = getItemBranches(record.d[1]['h_df852']);
...
var getItemAuthor = function(record) {
    try {
        return getProperty(getItemData(record, 'd', 0, 'df100'), 'df100', 'a');
    } catch (e) {
        return null;
    }
};
...
var getItemBranches = function(branches) {
    ...
    var _createBranch = function(data) {
        var branch = {
            'code': getProperty(data, 'h_df852', 'b'),
            'type': getProperty(data, 'h_df852', '2'),
            'name': getProperty(data, 'h_df852', '9')
        };
