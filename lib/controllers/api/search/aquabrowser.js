...
var _createResourceModel = function(isDetailRequest, record, callback) {
    try {
        ...
        var src = null;
        var extId = null;
        var title = _getResourceTitles(record);
        var isbn = null;
        ...
        if (isDetailRequest) {
            _getItemAvailability(extId, function(error, branches) {
                ...
                var result = new ResultModel.Result(id, src, extId, title, isbn, null, null, null, author, published, subject, series, note, contentType, thumbnail, link, branches);
...
var _getResourceTitles = function(record) {
    // console.log('_getResourceTitles');
};
